...
def parse_date(date_text):
    ...
    # Try format 1
    single_day = re.search(r'le (\d{2}/\d{2}/\d{4}) de (\d{2}:\d{2}) à (\d{2}:\d{2})', date_text)
    if single_day:
...
def generate_rss(c, h):
    ...
    fg.description("Agenda des sorties clubs (Canyonisme) - Club Alpin Français Cannes-Côte d'Azur")
